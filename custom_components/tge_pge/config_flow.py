...
from .const import (
    DOMAIN, CONF_UNIT, UNIT_ZL_MWH, UNIT_GR_KWH, UNIT_ZL_KWH, CONF_STATE_TEMPLATE_FIXING_1_RATE,
     CONF_STATE_TEMPLATE_FIXING_1_VOLUME,
    PARAMETER_FIXING_1_RATE, PARAMETER_FIXING_1_VOLUME,
    CONF_USE_STATE_TEMPLATES)

_LOGGER = logging.getLogger(__name__)


class PgeTgeFlowHandler(config_entries.ConfigFlow, domain=DOMAIN):
    VERSION = 1
    ...
    async def async_step_user(self, user_input: dict[str, Any] | None = None) -> ConfigFlowResult:
        ...
        if user_input is not None:
            return self.async_create_entry(title="PGE TGE", data=user_input)
...
class PgeTgeOptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_templates(self, user_input: dict[str, Any] | None = None) -> ConfigFlowResult:
        errors = {}
        if user_input is not None:
            for template_conf_key in [CONF_STATE_TEMPLATE_FIXING_1_RATE,
                                      CONF_STATE_TEMPLATE_FIXING_1_VOLUME]:
    ...
    async def _update_options(self) -> ConfigFlowResult:
        ...
        return self.async_create_entry(
            title="PGE TGE", data=self.options
        )
